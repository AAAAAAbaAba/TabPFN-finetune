...
from train_utils.utils import set_locals_in_self, my_get_preprocessed_datasets
...
def get_batch_to_dataloader(get_batch_method_):
    class DL(PriorDataLoader):
        ...
        def __init__(self, num_steps, **get_batch_kwargs):
            ...
            self.epoch_count = 0
            self.collate_fn = get_batch_kwargs.get('collate_fn') or self.collate_fn
        ...
        def gbm(self, *args, eval_pos_seq_len_sampler, **kwargs):
            ...
            preprocessed_batch = my_get_preprocessed_datasets(
                self.model, x, y, 
                kwargs['single_eval_pos'] / kwargs['seq_len'], 
                "regressor",
            )
            return self.collate_fn([preprocessed_batch[0]])
        ...
        for d2 in range(0, data.shape[1]):
            ...
            if d == d2:
                if categorical:
                    ...
                    sns.kdeplot(data=df, x='value', hue='target', ax=sub_ax, legend=False, palette="deep")
                else:
                    # 原代码
                    # sns.kdeplot(data[:, d], ax=sub_ax, legend=False)
                    # 修改后的代码
                    sns.kdeplot(data=data[:, d], ax=sub_ax, legend=False)
